...
export function getColumnScrollLeft<R>(columns: CalculatedColumn<R>[], idx: number, currentScrollLeft: number, currentClientWidth: number): number {
  let left = 0;
  let frozen = 0;
...
export function getRowTop(rowIdx: number, rowHeight: number): number {
